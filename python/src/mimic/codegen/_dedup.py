...
def _skeleton_hash(node: WidgetNode) -> str:
    """Hash of the structural skeleton — kinds, styling, children shape.

    Ignores: ids, text content, image urls (which vary per instance).
...
def _style_signature(style: dict) -> dict:
    """Drop fields that are 'content-y' rather than structural."""
    out = dict(style)
    return out
